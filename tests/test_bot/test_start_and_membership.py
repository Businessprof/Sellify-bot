...
def test_format_welcome_message():
    ...
    text = format_welcome_message(
        ...
        referral_link="https://t.me/Qamify_Bot?start=ref_REF43F3E22A",
    )
    ...
    assert "$0.00" in text
    assert "https://t.me/Qamify_Bot?start=ref_REF43F3E22A" in text


def test_membership_gate_keyboard():
    channels = [
        {"title": "Join Channel", "link": "https://t.me/qamify_ch"},
        {"title": "Join Group", "link": "https://t.me/qamify_chat"},
    ]
    kb = get_membership_gate_keyboard(channels)
    assert len(kb.inline_keyboard) == 3
    assert kb.inline_keyboard[0][0].text == "📢 Join Channel"
    assert kb.inline_keyboard[0][0].url == "https://t.me/qamify_ch"
